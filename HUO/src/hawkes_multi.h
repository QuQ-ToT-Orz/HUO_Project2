#ifndef hawkes_multi_hpp
#define hawkes_multi_hpp
#undef TMB_OBJECTIVE_PTR
#define TMB_OBJECTIVE_PTR obj

/* Multi-Series Marked Univariate Hawkes Process - Shared Parameters Only */
template<class Type>
Type hawkes_multi(objective_function<Type>* obj) {
  ...
  PARAMETER(logit_abratio);    // Single alpha ratio for all series
  ...
  Type alpha = exp(logit_abratio) / (Type(1.) + exp(logit_abratio)) * (beta / overall_mean_marks);
  
  // Likelihood calculations - process each series separately 
